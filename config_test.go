...
// TestNewLogConfig 测试新配置创建
func (suite *ConfigTestSuite) TestNewLogConfig() {
	config := NewLogConfig()
...
func (suite *ConfigTestSuite) TestConfigEquality() {
	config1 := NewLogConfig().WithLevel(DEBUG).WithShowCaller(true)
	config2 := NewLogConfig().WithLevel(DEBUG).WithShowCaller(true)
...
func (suite *ConfigTestSuite) TestConfigWithDifferentLevels() {
	...
	for i, level := range levels {
		suite.T().Run(levelNames[i], func(t *testing.T) {
			config := NewLogConfig().WithLevel(level)
...
func (suite *ConfigTestSuite) TestConfigWithDifferentOutputs() {
	...
	for name, output := range outputs {
		suite.T().Run(name, func(t *testing.T) {
			config := NewLogConfig().WithOutput(output.(interface{ Write([]byte) (int, error) }))
...
func (suite *ConfigTestSuite) TestConfigTimeFormats() {
	...
	for _, format := range timeFormats {
		suite.T().Run(strings.ReplaceAll(format, ":", "_"), func(t *testing.T) {
			config := NewLogConfig().WithTimeFormat(format)
...
func (suite *ConfigTestSuite) TestConfigPrefixes() {
	...
	for name, prefix := range prefixes {
		suite.T().Run(name, func(t *testing.T) {
			config := NewLogConfig().WithPrefix(prefix)
